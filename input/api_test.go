...
func TestFetchWithAllSuccess(t *testing.T) {
	httpmock.Activate()
...
func TestFetchWithNoPatternSuccess(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
...
func TestFetchWithEvenSuccess(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
	...
	httpmock.RegisterResponder(
		"GET",
		`=~^http://example.com/todos/2`,
		httpmock.NewStringResponder(200, `{"id": 2, "userId": 1, "title": "Test TODO 2", "completed": false}`))

	httpmock.RegisterResponder(
		"GET",
		`=~^http://example.com/todos/4`,
		httpmock.NewStringResponder(200, `{"id": 4, "userId": 1, "title": "Test TODO 4", "completed": true}`))

	expected := []types.TODO{
		{ID: 2, UserID: 1, Title: "Test TODO 2", Completed: false},
		{ID: 4, UserID: 1, Title: "Test TODO 4", Completed: true},
	}
	api := input.API{URL: "http://example.com/todos"}

	result, err := api.Fetch(2, "even")
	callCount := httpmock.GetTotalCallCount()
	assert.Equal(t, 2, callCount)
...
func TestFetchWithOddSuccess(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
	...
	httpmock.RegisterResponder(
		"GET",
		`=~^http://example.com/todos/3`,
		httpmock.NewStringResponder(200, `{"id": 3, "userId": 1, "title": "Test TODO 3", "completed": true}`))

	expected := []types.TODO{
		{ID: 1, UserID: 1, Title: "Test TODO", Completed: false},
		{ID: 3, UserID: 1, Title: "Test TODO 3", Completed: true},
	}
	api := input.API{URL: "http://example.com/todos"}

	result, err := api.Fetch(2, "odd")
	callCount := httpmock.GetTotalCallCount()
	assert.Equal(t, 2, callCount)
